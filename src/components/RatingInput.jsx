...
// Icona stella SVG (mantenuta invariata)
...
export default function RatingInput({ gameId }) {
    ...
    // Converte l'ID del gioco in numero per una compatibilità ottimale con int8/bigint di Postgres
    ...
    useEffect(() => {
        ...
        const fetchRating = async () => {
            ...
            // RIMOSSO .single() per risolvere il 406. Gestiamo il risultato come ARRAY.
            const { data: records, error } = await supabase
                .from('ratings')
                .select('id, rating_value')
                .eq('user_id', user.id)
                .eq('game_id', numericGameId) // Usiamo l'ID convertito
                .limit(1); // Manteniamo il limite per efficienza

            if (error) {
                 // PGRST116 (No rows found) non viene più lanciato, gestiamo solo errori veri
        ...
        fetchRating();
    }, [user, numericGameId]); // Dipendenza da numericGameId
  ...
  console.log("Utente Loggato (user.id):", user.id);
    ...
    console.log("Valore voto (ratingValue):", ratingValue);
        ...
        if (ratingId) {
            ...
            // Qui possiamo tenere .single() perché ci aspettiamo esattamente 1 risultato (quello appena creato)
